        ...
        class VitalsMinigameUpdatePatch
        {
            static bool Prefix(VitalsMinigame __instance)
            {
                ...
                if (MapOptionsTor.restrictDevices > 0)
                {
                    ...
                    if (MapOptionsTor.restrictVitalsTime <= 0f && CachedPlayer.LocalPlayer.PlayerControl != Hacker.hacker && CachedPlayer.LocalPlayer.PlayerControl != PhantomRole.phantomRole && !CachedPlayer.LocalPlayer.Data.IsDead)
                    {
                        __instance.Close();
                        return false;
                    }
