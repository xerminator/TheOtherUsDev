        ...
        class VitalsMinigameUpdatePatch
        {
            static bool Prefix(VitalsMinigame __instance)
            {
                ...
                if (MapOptions.restrictDevices > 0)
                {
                    ...
                    if (MapOptions.restrictVitalsTime <= 0f && CachedPlayer.LocalPlayer.PlayerControl != Hacker.hacker && CachedPlayer.LocalPlayer.PlayerControl != PhantomRole.phantomRole && !CachedPlayer.LocalPlayer.Data.IsDead)
                    {
                        __instance.Close();
                        return false;
                    }
